...
- **Symmetric collectives** (``all_reduce``, ``all_gather``, ``reduce_scatter``):
  use the **minimum** total time across ranks.  These collectives transfer
  ...
  true transfer time; earlier ranks include wait time.
...
_COMM_KEYWORDS = (
    "cross_device_reduce", "all_reduce", "all_gather",
    "ncclkernel", "nccldev", "alltoall", "all_to_all",
)
...
def classify_kernel(
    ...
    if "argmax" in nl:
        return "sampler"
    if ("copy_kernel" in nl or "catarraybatchedcopy" in nl
            or "memcpy" in nl or "fillfunctor" in nl):
        return "copy"
...
def _read_rank_stats(
    ...
    with open(csv_path, newline="") as f:
        ...
        for row in reader:
            ...
            cats[cat] += dur
    if skipped:
        print(f"  [warn] {os.path.basename(csv_path)}: skipped {skipped} rows with missing Duration")
    return dict(cats)
...
def aggregate(
    ...
    *,
    ...
    # Build result: comm → min or max (see _comm_agg_method); compute → mean
    result_cats: dict[str, dict] = {}
    for cat in all_cats:
        vals = [s.get(cat, 0) for s in rank_stats]
        if is_comm(cat):
            method = _comm_agg_method(cat)
            chosen = max(vals) if method == "max" else min(vals)
            result_cats[cat] = {
                "us": round(chosen, 1),
                "method": f"{method}-across-ranks",
...
def print_result(result: dict) -> None:
    ...
    nr = result["num_ranks"]
    print(f"\n{'=' * 60}")
    print(f"  {stage}  (corrected, {nr} ranks, sym-min / asym-max / compute-mean)")
...
def _build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        description="Aggregate kernel stats across TP/DP ranks (sym-comm: min, asym-comm: max, compute: mean).",
...
def main(argv: Optional[list] = None) -> int:
    ...
    result = aggregate(args.csv_dir, stage=args.stage, compute_only=args.compute_only)
    if not result:
        return 1
